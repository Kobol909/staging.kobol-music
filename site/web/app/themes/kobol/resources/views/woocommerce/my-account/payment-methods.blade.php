		...
		<thead>
			<tr>
				@foreach ( wc_get_account_payment_methods_columns() as $column_id => $column_name )
					<th class="woocommerce-PaymentMethod woocommerce-PaymentMethod--@php echo esc_attr( $column_id ); @endphp payment-method-@php echo esc_attr( $column_id ); @endphp"><span class="nobr"><?php echo esc_html( $column_name ); ?></span></th>
				@endforeach
			</tr>
		</thead>
		@foreach ( $saved_methods as $type => $methods ) 
		{{-- phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibite --}}
			@foreach ( $methods as $method ) 
				<tr class="payment-method @php echo ! empty( $method['is_default'] ) ? ' default-payment-method' : ''; @endphp">
					@foreach ( wc_get_account_payment_methods_columns() as $column_id => $column_name ) 
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--@php echo esc_attr( $column_id ); @endphp payment-method-@php echo esc_attr( $column_id ); @" data-title="@php echo esc_attr( $column_name ); @endphp">
							@php
...
@if ( WC()->payment_gateways->get_available_payment_gateways() ) 
	<a class="button" href="@php echo esc_url( wc_get_endpoint_url( 'add-payment-method' ) ); @endphp">@php esc_html_e( 'Add payment method', 'woocommerce' ); @endphp</a>
@endif
