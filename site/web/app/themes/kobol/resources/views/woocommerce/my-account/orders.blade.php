		...
		<thead>
			<tr>
				@foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) 
					<th class="woocommerce-orders-table__header woocommerce-orders-table__header- @php echo esc_attr( $column_id ); @endphp"><span class="nobr">{!! echo esc_html( $column_name ); !!}</span></th>
				@endforeach
			</tr>
		</thead>
		...
		<tbody>
			@foreach ( $customer_orders->orders as $customer_order )
				...
				<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status- @php echo esc_attr( $order->get_status() ); @endphp order">
					@foreach ( wc_get_account_orders_columns() as $column_id => $column_name ) 
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell- @php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); @endphp">
							...
							@elseif ( 'order-number' === $column_id ) 
								<a href="@php echo esc_url( $order->get_view_order_url() ); @endphp">
									@php echo esc_html( _x( '#', 'hash before order number', 'woocommerce' ) . $order->get_order_number() ); @endphp
								</a>

							@elseif ( 'order-date' === $column_id ) 
								<time datetime="@php echo esc_attr( $order->get_date_created()->date( 'c' ) ); @endphp">@php echo esc_html( wc_format_datetime( $order->get_date_created() ) ); @endphp</time>
